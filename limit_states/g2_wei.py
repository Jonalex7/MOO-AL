...
class g2d_wei_g2():
    """Wei et al. (Eq. 20b) benchmark limit-state function.

    g2(x1, x2) = 10.7 - sum_{i=1..2} (x_i^2 - 5*cos(0.8*pi*x_i))
    """

    def __init__(self):
        self.input_dim = DIM
        self.output_dim = 1
        self.target_pf = 0.07175746
    ...
    def eval_lstate(self, x):
        ...
        xi = x[:, :2]
        g_val_sys = 10.7 - np.sum(xi ** 2 - 5.0 * np.cos(0.8 * np.pi * xi), axis=1)
        return g_val_sys
