...
class AcquisitionStrategy:
    ...
    #     return gamma
    # --- Updated logistic_gamma function ---
    def logistic_gamma(self, delta_P, avg_norm_std, delta_P0=0.2, k=40):
        gamma_beta = 0.05

        # Dynamic cap: gamma_limit = self.gamma_max * (1.0 - discount)
        gamma_limit = self.gamma_max - gamma_beta * avg_norm_std
        
        # Ensure a small numerical floor to avoid zero division if needed(pure exploitation).
        gamma_limit = max(1e-2, gamma_limit) 
        print(f'gamma_limit: {gamma_limit:.3f}')
        # Use the dynamic gamma_limit in the logistic function
        gamma = gamma_limit * (1 / (1 + np.exp(-k * (delta_P - delta_P0))))
        return gamma
    ...
    def get_moo_reliability(self, pareto_front, pf_estimate):
        ...
        avg_norm_std = normalized_std.mean().item()
        print(f'avg_norm_std: {avg_norm_std:.3f}')
        # compute gamma and update Pf_prev
        # gamma = self.logistic_gamma(delta_avg, delta_P0=self.delta_P0, k=self.k_balance)
        gamma = self.logistic_gamma(
                                    delta_avg, 
                                    avg_norm_std, # Pass the new metric
                                    delta_P0=self.delta_P0, 
                                    k=self.k_balance
                                                    )
