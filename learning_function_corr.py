...
config={}
#loading data set
case_study = config['case_study']  = 'corr'
...
for active in num_active_samples:
    ...
    for seed in seeds_exp:
        ...
        for ep in range(active_loop):
            ...
            mean_prediction, std_prediction = gaussian_process.predict(x_mc_pool, return_std=True, return_cov=False)
            mean_pred = torch.tensor(mean_prediction)
            std_pred = torch.tensor(std_prediction)

            #----
            print(f'Pf estimation... all dataset')
            mean_mc, _ = gaussian_process.predict(dataset_norm.input_tensor, return_std=True, return_cov=False)
            mean_mc = torch.tensor(mean_mc)
            # # -----------------------------------------------
            Pf_model = torch.sum(mean_mc < 0) / len(mean_mc)
            ...
            _, u_idx = u_function.topk(1, largest=False)
            list_u_idx.append(u_idx.item())   #indices of U already selected
            selected_idx_pool = [idx_pool[u_idx.item()]]

            print(f'Selected samples:', end=" ")
            for sample in range(active_samples-1):
                print(f'{len(selected_idx_pool)+1}', end=" ")
                #Covariance computation 
                det_cov = []
                
                for sample in range(len(idx_pool)):
                    x_assemble = dataset_norm.input_tensor[selected_idx_pool + [idx_pool[sample]]]
                    _, cov_  = gaussian_process.predict(x_assemble, return_std=False, return_cov=True)
                ...
                u_function[list_u_idx] = torch.inf
                #minimum U
                _, u_idx = u_function.topk(1, largest=False)
                idx_cov_select = idx_pool[u_idx.item()]

                # selected_idx_pool
                selected_idx_pool.append(idx_cov_select)
                list_u_idx.append(u_idx.item())  #indices of U already selected
